...
def render_case(case_dir: str, output_path: str,
                width=1280, height=720, fps=60, xml_path: str = None):
    """
    ...
    # ── Load MuJoCo model ──
    model = mj.MjModel.from_xml_path(xml_path or XML_PATH)
...
def main():
    ...
    for i, case_dir in enumerate(case_dirs):
        ...
        per_case_xml = REPO / "models" / "MJCF" / "go2" / "environments" / f"{case_dir.name}.xml"
        xml_for_case = str(per_case_xml) if per_case_xml.exists() else None
        if xml_for_case:
            print(f"  Using per-case XML: {per_case_xml.name}")

        try:
            render_case(str(case_dir), output_path,
                        width=args.width, height=args.height, fps=args.fps,
                        xml_path=xml_for_case)
        except Exception as e:
            print(f"  ERROR: {e}")
