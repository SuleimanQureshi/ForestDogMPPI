...
class SimpleMPPI:
    def __init__(self, dt):
        ...
        self.vx_limits = (-0.8, 0.8)
        self.vy_limits = (-0.3, 0.3)
        self.wz_limits = (-0.8, 0.8)
...
traj.N = int(GAIT_T / MPC_DT)
N = traj.N
dt = MPC_DT
# go2.update_model()  # ensure Ig is valid
traj.m = go2.data.Ig.mass
traj.I_com_world = go2.data.Ig.inertia

traj.pos_traj_world = np.zeros((3, N))
traj.vel_traj_world = np.zeros((3, N))
traj.rpy_traj_world = np.zeros((3, N))
traj.omega_traj_world = np.zeros((3, N))

traj.pos_traj_world[2, :] = 0.27

traj.contact_table = gait.compute_contact_table(0.0, dt, N)

# Dummy foot lever arms (zero is fine for structure build)
traj.r_fl_foot_world = np.zeros((3, N))
traj.r_fr_foot_world = np.zeros((3, N))
traj.r_rl_foot_world = np.zeros((3, N))
traj.r_rr_foot_world = np.zeros((3, N))

traj._continuousDynamics(go2)
...
# )
traj.N = int(GAIT_T / MPC_DT)
traj.initial_x_vec = go2.compute_com_x_vec()
    ...
    for k in range(SIM_STEPS):
        ...
        if (k % CTRL_DECIM) == 0 and ctrl_i < CTRL_STEPS:
            ...
            if (ctrl_i %( 5* STEPS_PER_MPC)) == 0:
                ...
                # Convert to WORLD frame using current yaw
                Rz = np.array([
                    [np.cos(yaw), -np.sin(yaw), 0.0],
                    [np.sin(yaw),  np.cos(yaw), 0.0],
                    [0.0,          0.0,         1.0],
                ])
                v_world = Rz @ np.array([vx_des_body, vy_des_body, 0.0], dtype=float)

                go2.x_vel_des_world = float(v_world[0])
                go2.y_vel_des_world = float(v_world[1])
                go2.yaw_rate_des_world = float(wz_des_body)
                N = traj.N
                dt = MPC_DT
                ...
                for i in range(N):
                    u = u0
                    state[0] += (u[0]*np.cos(state[2]) - u[1]*np.sin(state[2])) * dt
                    state[1] += (u[0]*np.sin(state[2]) + u[1]*np.cos(state[2])) * dt
                    state[2] += u[2] * dt

                    traj.pos_traj_world[0, i] = state[0]
                    traj.pos_traj_world[1, i] = state[1]
                    traj.pos_traj_world[2, i] = 0.27

                    traj.vel_traj_world[0, i] = u[0]
                    traj.vel_traj_world[1, i] = u[1]
                    traj.vel_traj_world[2, i] = 0.0

                    traj.rpy_traj_world[2, i] = state[2]
                    traj.omega_traj_world[2, i] = u[2]
                    # --- Provide desired COM position to gait (used for swing foot placement correction) ---
                    # Use the first step of the reference (small, local) and clamp like original ComTraj did.
                    x_des = float(traj.pos_traj_world[0, 0])
                    y_des = float(traj.pos_traj_world[1, 0])

                    max_pos_error = 0.1
                    x_des = float(np.clip(x_des, px - max_pos_error, px + max_pos_error))
                    y_des = float(np.clip(y_des, py - max_pos_error, py + max_pos_error))

                    go2.x_pos_des_world = x_des
                    go2.y_pos_des_world = y_des


                traj.contact_table = gait.compute_contact_table(time_now_s, dt, N)
                # these used to be set inside generate_traj()
                traj.initial_x_vec = go2.compute_com_x_vec()
                traj.m = go2.data.Ig.mass
                traj.I_com_world = go2.data.Ig.inertia
                traj._continuousDynamics(go2)
